...
Namespace AcCommon
    Public Module Layers
        ...
        Public Sub FormLayersXML(lyrs As AcdLayers)
            Dim curDwg As Document = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument
            ...
            If File.Exists(xmlNameStr) Then Kill(xmlNameStr)
        ...
        Public Sub MergeThenDeleteLayer(lName As String, mergeLayer As String, Optional removeLayer As Boolean = False)
            ...
            Using acTrans As Transaction = dwgDB.TransactionManager.StartTransaction()
                ...
                If acLyrTbl.Has(lName) Then
                    If acLyrTbl.Has(mergeLayer) Then
                        ...
                        If mergeList IsNot Nothing AndAlso mergeList.Count > 0 Then
                            ...
                            If uR = True Then
                                For k As Integer = 0 To mergeList.Count - 1
                                    ...
                                    End If
                                Next
                                If removeLayer Then DeleteMyLayer(lName, True)
                            Else
                                Exit Sub
                            End If
                    ...
                    Else
                        MessageBox.Show("Merge layer does not exist in current drawing.  Ending command.")
                        Throw New Exception(ErrorStatus.AmbiguousInput)
                    End If

                Else
                    MessageBox.Show("Layer does not exist in current drawing.  Ending command.")
                    Throw New Exception(ErrorStatus.AmbiguousInput)
                End If
        ...
        End Sub
        Public Sub DeleteMyLayer(lName As String, Optional silent As Boolean = False)
        ...
        Public Sub NormalPstyleToLayer(ByVal layID As ObjectId)
            ...
            If dwgDB.PlotStyleMode = False Then
                ...
                Using trans As Transaction = dwgDB.TransactionManager.StartTransaction
                    ...
                    Dim ltr As LayerTableRecord = TryCast(trans.GetObject(layID, OpenMode.ForWrite), LayerTableRecord)
        ...
        Friend Function GetLayerSchemaSet(xSDfileName As String, Optional xmlNameSpace As String = "") As XmlSchemaSet
            ...
            'create a temporary text file to use for xsd validation
            ...
            Try
                ...
                Dim rdr As TextReader = New StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("MasterCustomLibrary.LayerSchema1.xsd"))
            ...
            Catch e As Exception
                Try
                    If String.IsNullOrEmpty(xSDfileName) Then xSDfileName = GetMyXSDFileName()
                    If String.IsNullOrEmpty(xSDfileName) Then
                    ...
                    End If
                Catch
                    MessageBox.Show("No XSD file or invalid XSD file specified.")
                    Return Nothing
                    Exit Function
                End Try

            Finally
                If File.Exists(tmpFilePath) Then Kill(tmpFilePath)
            End Try
    ...
    Public Module FileUtilities
        ...
        Public Function GetEmbeddedResource(ResourceName As String) As String
            ...
            Dim tmpFilePath As String = Path.GetTempFileName()
            'Dim tmpFilePath As String = "H:\VS Repos\MasterCustomLbry\Blockschema.xsd"
            Try
                'create a textreader object to read the file embedded in the Kinematics assembly
                Dim rdr As TextReader = New StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream(ResourceName))
                'use a textwriter object to write the XSD file to the temporary file
                Dim wr As TextWriter = New StreamWriter(tmpFilePath, False)
                wr.WriteLine(rdr.ReadToEnd())
                wr.Close()
            ...
            Catch e As Exception
                MessageBox.Show("No embedded file or invalid file specified.")
                Return Nothing
                Exit Function
            End Try
...
skipit:
        ...
        Public Function GetASaveFileName(ext As String, Optional ext2 As String = "") As String

            'function for getting filename
            Dim curDwg As Document = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument
            Dim dwgName As String = curDwg.Name
            Dim folderName As String = Path.GetDirectoryName(dwgName)
            Dim allFiles As Boolean = True
            ...
            Try
                ...
                filterString = ext & " Files (*." & ext & ")|*." & ext
                If Not String.IsNullOrEmpty(ext2) Then
                    filterString &= "|" & ext2 & " Files (*." & ext2 & ")|*." & ext2
                End If
...
tryAgain:
    ...
    Public Module Properties
        ...
        Public Function GetPlotStyleID(psName As String) As ObjectId
            ...
            Using actrans As Transaction = dwgDB.TransactionManager.StartTransaction
                Dim psDic As DictionaryWithDefaultDictionary = actrans.GetObject(dwgDB.PlotStyleNameDictionaryId, OpenMode.ForRead)
    ...
    Public Module MathGeometry
        Public Function ConvertPoints2d(pts3d As Point3dCollection, Optional refPln As Plane = Nothing) As Point2dCollection
            'by David Eisenbeisz (c)2024
            'converts point3d collection to point2d collection using designated plane or WCS xy plane

            Dim pts2d As New Point2dCollection

            If refPln Is Nothing Then refPln = New Plane(New Point3d(0, 0, 0), Vector3d.ZAxis)

            If pts3d IsNot Nothing And pts3d.Count > 0 Then
                For i = 0 To pts3d.Count - 1
                    Dim pt2d As Point2d = pts3d(i).Convert2d(refPln)
                    pts2d.Add(pt2d)
                Next
        ...
        Public Function GetTangentBulge(cArc As CircularArc2d) As Double
            'get tangent bulge of circular arc

            Dim startAng As Double = cArc.StartAngle
            Dim endAng As Double = cArc.EndAngle
            Dim delta As Double = endAng - startAng
            Dim blg As Double = Tan(delta / 4)

            Return blg

        End Function
        Public Function GetTangentBulge(startAng As Double, endAng As Double) As Double
            'get tangent bulge of circular arc
        ...
        Public Function OffsetSideByPoint(clineID As ObjectId, offdist As Double, Optional PrptMessage As String = "", Optional refPt As Point3d = Nothing) As Integer
            ...
            Using acTrans As Transaction = dwgDB.TransactionManager.StartTransaction()
                ...
                Try
                    ...
                    If pl IsNot Nothing Then
                        ...
                        Dim cp As Point3d = pl.GetClosestPointTo(pRef, False)
        ...
        Public Function UnitVector2d(vect As Vector2d) As Vector2d
            ...
            Try
                'UnitVector components
                Dim xnrml As Double = vect.X / (vect.X ^ 2 + vect.Y ^ 2) ^ (0.5)
                Dim ynrml As Double = vect.Y / (vect.X ^ 2 + vect.Y ^ 2) ^ (0.5)
                'unit vector
                unitVect = New Vector2d(xnrml, ynrml)
            Catch ex As Exception
                Return Nothing
                Exit Function
            End Try
        ...
        Public Function MakeNewUCS(orgPt As Point3d, xAxPt As Point3d, ucsName As String) As Boolean
            ...
            Using acTrans As Transaction = DwgDB.TransactionManager.StartTransaction()
                Try
                    ...
                    If Not tempXaxis.IsPerpendicularTo(tempZaxis) Then 'And xAxUnitVect.IsPerpendicularTo(zAxUnitVect) And yAxUnitVect.IsPerpendicularTo(zAxUnitVect) Then
                        MessageBox.Show("Not orthogonal. Dot product between x and z axes = " & tempValue.ToString)
                        Return Nothing
                        Exit Function
                    End If
        ...
        Public Function GetUCSMatrix3D(orgPt As Point3d, xAxPt As Point3d) As Matrix3d
            ...
            Try
                Using acTrans1 As Transaction = DwgDB.TransactionManager.StartTransaction()
                    ...
                    tempMat3d.ElementAt(0, 2) = znrmlx 'should be zero for plan-view transformations
...
TryAgain:
    ...
    Public Module OtherMethods
        Public Function LtIsLoaded(ltName As String) As Boolean
            ...
            Using acTrans As Transaction = dwgDB.TransactionManager.StartTransaction
                Dim ltTbl As LayerTable = acTrans.GetObject(dwgDB.LayerTableId, OpenMode.ForRead)
                If ltTbl.Has(ltName) Then
                    Return True
                Else
                    Return False
        ...
        Public Function IsInLayoutPaper() As Boolean
            ...
            If dwgDB.TileMode Then
                Return False
            Else
                If dwgDB.PaperSpaceVportId = ObjectId.Null Then
                    Return False
                ElseIf ed.CurrentViewportObjectId = ObjectId.Null Then
                    Return False
                ElseIf ed.CurrentViewportObjectId = dwgDB.PaperSpaceVportId Then
                    Return True
                Else
                    Return False
                End If
        ...
        Public Function YesNoQuery(msgStr As String) As Boolean
            ...
            Dim pko As New PromptKeywordOptions(msgStr)
            With pko
                .Keywords.Add("Yes")
                .Keywords.Add("No")
                .AllowArbitraryInput = False
        ...
        Friend Function GetZoomWindow(ByVal entId As ObjectId) As Array()
            ...
            Dim len As Double = Math.Max(h, w) / 4.0

            Dim pt1(2) As Double
            pt1(0) = ext.MinPoint.X - len
            pt1(1) = ext.MinPoint.Y - len
            pt1(2) = ext.MinPoint.Z

            Dim pt2(2) As Double
            pt2(0) = ext.MaxPoint.X + len
            pt2(1) = ext.MaxPoint.Y + len
            pt2(2) = ext.MaxPoint.Z

            Dim corners(1) As Array
            corners(0) = pt1
            corners(1) = pt2
        ...
        Friend Function GetMyEntity(tp As Type, addmsg As String, Optional rejectMsg As String = "", Optional exactMatch As Boolean = False) As ObjectId
            ...
            Dim peo As New PromptEntityOptions(addmsg)
            With peo
                .AddAllowedClass(tp, exactMatch)
                If Not String.IsNullOrEmpty(rejectMsg) Then .SetRejectMessage(rejectMsg)
                .AllowNone = False
            End With
